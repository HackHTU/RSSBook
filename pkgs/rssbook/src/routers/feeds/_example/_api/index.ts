...
/**
 * `_api` is an example source that fetches data from an API.
...
export default new Source({
	...
	slug: "github", // [IMPORTANT]: Source unique identifier, must be lowercase letters and hyphens
	// [IMPORTANT]: 源的唯一标识符，必须是小写字母和短横线组成

	title: "Github", // Source title / 源的标题
})
		...
		(app) =>
			app
				...
				.get(
					...
					async ({
						// props
						meta: { domain }, // meta contains all metadata for the source and Feed
						// meta 包含了源和 Feed 的所有元数据
						params: { username }, // Get route parameter user / 获取路由参数 user

						// function
						cache, // cache is a cache, you can use it to store and reuse data
						// cache 是一个缓存，你可以用它来存储和复用数据
						date, // date is a date formatting function / date 是一个日期格式化函数
						ofetch, // ofetch is an enhanced fetch function / ofetch 是一个增强版的 fetch 函数
					}) => {
						const rootURL = `https://api.${domain}`;
						const link = `${rootURL}/users/${username}/events`;

						// 1. Fetch data via API, and use cache to avoid repeated fetching
						...
						// 2. Construct and return the Data object
						// 2. 构造 Data 对象并返回
						const data: Data = {
 ...
 * In practice, you can place this in `types/*.ts` files for centralized type management.
 *
 * 一个简单的 GitHub Event 接口
